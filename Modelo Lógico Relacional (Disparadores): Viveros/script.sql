...
DECLARE
    correo VARCHAR(45);
BEGIN
    IF NEW.email IS NULL OR NEW.email NOT SIMILAR TO '[\w|\.]+@([\w-]+\.)+[\w-]{2,4}' THEN
      RAISE NOTICE 'Creando email para el usuario con DNI: %', NEW.DNI;
...
CREATE OR REPLACE FUNCTION control_viviendas()
    RETURNS trigger AS $viviendas$
BEGIN
    IF NEW.viviendas >= 2 AND NEW.municipio = 'Catastro' THEN
        RAISE 'No pueden tener dos viviendas';
    END IF;
    RETURN NEW;
END;
$viviendas$ LANGUAGE 'plpgsql';
...
CREATE TRIGGER trigger_control_viviendas
  AFTER INSERT
  ON viveros.empleado
  FOR EACH ROW
  EXECUTE PROCEDURE control_viviendas();
...
INSERT INTO viveros.empleado (DNI, sueldo, telefono, zona_identificador, zona_latitud_vivero, zona_longitud_vivero, municipio, viviendas) values ('02203658A', 1000, 614572983, 1, 28.5225, -16.337944, 'Catastro', 1);
INSERT INTO viveros.empleado (DNI, sueldo, telefono, zona_identificador, zona_latitud_vivero, zona_longitud_vivero, municipio, viviendas) values ('09841512B', 1000, 614572984, 1, 28.3556667, -16.370583, 'Santa Cruz de Tenerife', 3);
INSERT INTO viveros.empleado (DNI, sueldo, telefono, zona_identificador, zona_latitud_vivero, zona_longitud_vivero, municipio, viviendas) values ('89645151C', 1000, 614572985, 1, 28.3556667, -16.370583, 'San Cristóbal de La Laguna', 1);
INSERT INTO viveros.empleado (DNI, sueldo, telefono, zona_identificador, zona_latitud_vivero, zona_longitud_vivero, municipio, viviendas) values ('65168465D', 1000, 614572986, 1, 28.5225, -16.337944, 'Catastro', 2);
...
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion, apellido1, apellido2, email) values ('89465134X', 36.35, 6.55, 'Sanz', 'Herranz', 's.herranz@yahoo.es');
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion, nombre, email) values ('89465135W', 37.25, 7.15, 'Aritz',NULL);
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion, nombre, apellido2, email) values ('89465136V', 39.15, 9.15, 'Cristina', 'Batista', '123@gmail.com');
