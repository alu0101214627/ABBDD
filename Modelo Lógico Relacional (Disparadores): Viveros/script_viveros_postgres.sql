...
CREATE OR REPLACE FUNCTION crear_email--(IN dominio varchar)
  RETURNS TRIGGER AS
$$
DECLARE
    email VARCHAR(45);
BEGIN
    email := CONCAT (dni, '@', TG_ARGV[0]);
    IF nombre IS NOT NULL THEN
        IF apellido1 IS NOT NULL THEN
            email := CONCAT (nombre, apellido1, '@', TG_ARGV[0]);
        ELSE
            IF apellido2 IS NOT NULL THEN
                email := CONCAT (nombre, apellido2, '@', TG_ARGV[0]);
            ELSE
                RAISE NOTICE 'creado correo por defecto %', now();
            END IF;
        END IF;
    ELSE 
        IF apellido1 IS NOT NULL THEN
            IF apellido2 IS NOT NULL THEN
                email := CONCAT (apellido1, apellido2, '@', TG_ARGV[0]);
            ELSE
                RAISE NOTICE 'creado correo por defecto %', now();
            END IF;
        ELSE
...
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion) values ('89465132Z', 34.65, 4.35);
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion) values ('89465133Y', 35.45, 5.25);
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion) values ('89465134X', 36.35, 6.55);
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion) values ('89465135W', 37.25, 7.15);
INSERT INTO viveros.cliente_club (DNI, credito_mensual, bonificacion) values ('89465136V', 39.15, 9.15);
